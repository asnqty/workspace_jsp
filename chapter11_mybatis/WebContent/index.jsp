...
<script type="text/javascript">
	...
	function view_one(f){
		...
		// 를 통해 id만 전달이 가능하다.
		// 밑에 수정을 할 때 id와 pw를 이런식으로 넣어서 같이 던질 수도 있다. (hidden 속성으로 전달하지 않아도 되긴 함)
	}
	function insert(f){
	...
	function remove(f){
		...
		// remove.jsp
		if(!f.id.value){
			alert("데이터 삭제를 위해선 아이디를 입력해주세요.")
			f.id.focus();
			return;
		}
		if(!f.pw.value){
			alert("데이터 삭제를 위해선 비밀번호를 입력해주세요.")
			f.pw.focus();
			return;
		}
		f.action = 'remove.jsp';
	...
	function update_page(f){
		if(!f.id.value){
			alert("데이터 수정을 위해선 아이디를 입력해주세요.")
			f.id.focus();
			return;
		}
		if(!f.pw.value){
			alert("데이터 수정을 위해선 비밀번호를 입력해주세요.")
			f.pw.focus();
			return;
		}
