...
public class PersonAction {
	public String process(HttpServletRequest request) {
		...
		// session에 담을 때에는 결과를 확인하기 위해서 "session's "를 값 앖에 붙인다.
		...
		vo.setName("vo's " + name);
		vo.setAge("vo's " + age);
		vo.setPhone("vo's " + phone);
		vo.setAddr("vo's " + addr);
		vo.setSelf("vo's " + self);
